...
public abstract class StarLightEngine {
    ...
    protected static final ExtendedBlockStorage EMPTY_CHUNK_SECTION = new ExtendedBlockStorage(0,true);
    ...
    protected final void setupCaches(final IExtendedChunkProvider chunkProvider, final int centerX, final int centerY, final int centerZ,
        ...
        for (int dz = -radius; dz <= radius; ++dz) {
            for (int dx = -radius; dx <= radius; ++dx) {
                ...
                //final Chunk chunk = (Chunk)chunkProvider.getChunkForLight(cx, cz); // mappings are awful here, this is the "get chunk at if at least features"
                final Chunk chunk = (Chunk)chunkProvider.getLoadedChunk(cx, cz); // mappings are awful here, this is the "get chunk at if at least features"
    ...
    protected final void updateVisible(final IExtendedChunkProvider lightAccess) {
    ...
    protected final IBlockState getBlockState(final int worldX, final int worldY, final int worldZ) {
        ...
        if (section != null) {
            return section == EMPTY_CHUNK_SECTION ? AIR_BLOCK_STATE : section.get(worldX & 15, worldY & 15, worldZ & 15);
        }

        return null;
    }

    protected final IBlockState getBlockState(final int sectionIndex, final int localIndex) {
        final ExtendedBlockStorage section = this.sectionCache[sectionIndex];

        if (section != null) {
            return section == EMPTY_CHUNK_SECTION ? AIR_BLOCK_STATE : section.getData().get(localIndex);
        }
    ...
    public final void blocksChangedInChunk(final IExtendedChunkProvider lightAccess, final int chunkX, final int chunkZ,
    ...
    protected abstract void propagateBlockChanges(final IExtendedChunkProvider lightAccess, final Chunk atChunk, final Set<BlockPos> positions);

    protected abstract void checkBlock(final IExtendedChunkProvider lightAccess, final int worldX, final int worldY, final int worldZ);
    ...
    protected abstract int calculateLightValue(final IExtendedChunkProvider lightAccess, final int worldX, final int worldY, final int worldZ,
    ...
    protected void checkChunkEdge(final IExtendedChunkProvider lightAccess, final Chunk chunk,
        ...
        for (final AxisDirection direction : ONLY_HORIZONTAL_DIRECTIONS) {
            ...
            final VariableBlockLightHandler customLightHandler = ((ExtendedWorld)lightAccess).getCustomLightHandler();
    ...
    protected void checkChunkEdges(final IExtendedChunkProvider lightAccess, final Chunk chunk, final ShortCollection sections) {
    ...
    protected void checkChunkEdges(final IExtendedChunkProvider lightAccess, final Chunk chunk, final int fromSection, final int toSection) {
        final ChunkPos chunkPos = chunk.getPos();
        final int chunkX = chunkPos.x;
        final int chunkZ = chunkPos.z;
    ...
    protected final void propagateNeighbourLevels(final IExtendedChunkProvider lightAccess, final Chunk chunk, final int fromSection, final int toSection) {
        final ChunkPos chunkPos = chunk.getPos();
        final int chunkX = chunkPos.x;
        final int chunkZ = chunkPos.z;
    ...
    public final void forceHandleEmptySectionChanges(final IExtendedChunkProvider lightAccess, final Chunk chunk, final Boolean[] emptinessChanges) {
    ...
    public final void handleEmptySectionChanges(final IExtendedChunkProvider lightAccess, final int chunkX, final int chunkZ,
    ...
    protected final boolean[] handleEmptySectionChanges(final IExtendedChunkProvider lightAccess, final Chunk chunk,
                                                        final Boolean[] emptinessChanges, final boolean unlit) {
        final World world = (World)lightAccess;
        final int chunkX = chunk.getPos().x;
        final int chunkZ = chunk.getPos().z;
    ...
    public final void checkChunkEdges(final IExtendedChunkProvider lightAccess, final int chunkX, final int chunkZ) {
    ...
    public final void checkChunkEdges(final IExtendedChunkProvider lightAccess, final int chunkX, final int chunkZ, final ShortCollection sections) {
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, false);
        try {
            final Chunk chunk = this.getChunkInCache(chunkX, chunkZ);
            if (chunk == null) {
    ...
    protected abstract void lightChunk(final IExtendedChunkProvider lightAccess, final Chunk chunk, final boolean needsEdgeChecks);

    public final void light(final IExtendedChunkProvider lightAccess, final Chunk chunk, final Boolean[] emptySections) {
        final int chunkX = chunk.getPos().x;
        final int chunkZ = chunk.getPos().z;
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
    ...
    public final void relightChunks(final IExtendedChunkProvider lightAccess, final Set<ChunkPos> chunks,
        ...
        for (final ChunkPos chunkPos : chunks) {
            final int chunkX = chunkPos.x;
            final int chunkZ = chunkPos.z;
            final Chunk chunk = lightAccess.getLoadedChunk(chunkX, chunkZ);
            ...
            for (int i = 0, len = neighbourLightOrder.length; i < len; i += 2) {
                ...
                final Chunk neighbour = lightAccess.getLoadedChunk(neighbourX, neighbourZ);
                ...
                try {
                    ...
                    for (int dz2 = -1; dz2 <= 1; ++dz2) {
                        for (int dx2 = -1; dx2 <= 1; ++dx2) {
                            ...
                            final Chunk neighbour2 = lightAccess.getLoadedChunk(neighbourX2, neighbourZ2);
    ...
    protected final void performLightIncrease(final IExtendedChunkProvider lightAccess) {
        final World world = lightAccess.getWorld();
        long[] queue = this.increaseQueue;
        ...
        while (queueReadIndex < queueLength) {
            ...
            if ((queueValue & FLAG_HAS_SIDED_TRANSPARENT_BLOCKS) == 0L) {
                ...
                for (final AxisDirection propagate : checkDirections) {
                    ...
                    final IBlockState blockState = this.getBlockState(sectionIndex, localIndex);
                    if (blockState == null) {
                        continue;
                    }
                    ...
                    if (opacityCached != -1) {
                        ...
                        final int opacity = blockState.getLightOpacity(world, this.mutablePos1);
                ...
                final IBlockState fromBlock = this.getBlockState(posX, posY, posZ);
    ...
    protected final void performLightDecrease(final IExtendedChunkProvider lightAccess) {
        final World world = lightAccess.getWorld();
        long[] queue = this.decreaseQueue;
