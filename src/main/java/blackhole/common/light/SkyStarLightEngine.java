...
public final class SkyStarLightEngine extends StarLightEngine {
    ...
    protected boolean canUseChunk(final Chunk chunk) {
        ...
        return (this.isClientSide || chunk.isLightPopulated());
    }

    @Override
    protected void checkChunkEdges(final IExtendedChunkProvider lightAccess, final Chunk chunk, final int fromSection,
                                   final int toSection) {
    ...
    @Override
    protected void checkBlock(final IExtendedChunkProvider lightAccess, final int worldX, final int worldY, final int worldZ) {
    ...
    @Override
    protected int calculateLightValue(final IExtendedChunkProvider lightAccess, final int worldX, final int worldY, final int worldZ,
        ...
        switch ((int)this.getKnownTransparency(worldX, worldY, worldZ)) {
            ...
            default:
                ...
                opacity = Math.max(1, state.getLightOpacity((IBlockAccess) lightAccess, this.recalcCenterPos));
    ...
    @Override
    protected void propagateBlockChanges(final IExtendedChunkProvider lightAccess, final Chunk atChunk, final Set<BlockPos> positions) {
    ...
    @Override
    protected void lightChunk(final IExtendedChunkProvider lightAccess, final Chunk chunk, final boolean needsEdgeChecks) {
        this.rewriteNibbleCacheForSkylight(chunk);
        Arrays.fill(this.nullPropagationCheckCache, false);
            ...
            for (int currZ = minZ; currZ <= maxZ; ++currZ) {
                for (int currX = minX; currX <= maxX; ++currX) {
                    ...
                    for (int sectionY = highestNonEmptySection; sectionY >= 0; --sectionY) {
                        final ExtendedBlockStorage section = this.getChunkSection(currX >> 4, sectionY, currZ >> 4);
    ...
    protected final int tryPropagateSkylight(final IBlockAccess world, final int worldX, int startY, final int worldZ,
        ...
        for (;startY >= (this.minLightSection << 4); --startY) {
            ...
            IBlockState current = this.getBlockState(worldX, startY, worldZ);
            if (current == null) {
            ...
            if (opacityIfCached != -1) {
                ...
                final int opacity = current.getLightOpacity(world, mutablePos);
                if (opacity > 0) {
